...
export default function CheckoutPage({ orderId }: CheckoutPageProps) {
  ...
  const handleInitializePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.paystackAccessCode) {
        // Fallback to redirect if no access code (though we expect one)
      ...
      // Initialize Paystack Popup using setup()
      // Note: checking if setup exists, otherwise fallback or log
      if (typeof window.PaystackPop.setup !== 'function') {
        console.error(
          'PaystackPop.setup is not a function',
          window.PaystackPop
        );
        if (response.checkoutUrl) {
          window.location.href = response.checkoutUrl;
        ...
        throw new Error('Paystack SDK not loaded correctly');
      }
      ...
      // Fallback to redirect if popup fails and we have a URL
      if (typeof initializePayment.data?.checkoutUrl === 'string') {
        window.location.href = initializePayment.data.checkoutUrl;
        return;
      }
      alert(`Failed to initialize payment: ${(error as Error).message}`);
      setIsProcessing(false);
    }
  };
